...
export function validateForm(formData) {
  ...
  if (typeof phone !== "string" || phone.length !== 10 || !/^[6-9]/.test(phone) || /\D/.test(phone)) {
    errors.phone = "Invalid Indian phone number";
  }
  const age = formData.age;
  const ageNumber = typeof age === "string" ? parseInt(age) : age;
  if (isNaN(ageNumber) || !Number.isInteger(ageNumber) || ageNumber < 16 || ageNumber > 100) {
    errors.age = "Age must be an integer between 16 and 100";
  }
  const pincode = formData.pincode;
  if (typeof pincode !== "string" || pincode.length !== 6 || pincode.startsWith("0") || /\D/.test(pincode)) {
